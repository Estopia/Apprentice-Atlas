...
export default function AuthScreen() {
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.content}><Pressable accessibilityRole="button" accessibilityLabel={t(locale, 'actions.back')} onPress={() => router.back()} style={styles.backButton}><Text style={styles.back}>‹ {t(locale, 'actions.back')}</Text></Pressable><Text style={styles.title}>{t(locale, 'auth.title')}</Text><Text style={styles.copy}>{t(locale, 'auth.description')}</Text>{error && <Text accessibilityRole="alert" style={styles.error}>{error}</Text>}<AuthForm onSuccess={() => void complete()} /></View></SafeAreaView>;
}
const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#f7f5f0' }, content: { width: '100%', maxWidth: 900, alignSelf: 'center', padding: 24, gap: 10 }, backButton: { minHeight: 44, minWidth: 44, justifyContent: 'center', alignSelf: 'flex-start', marginBottom: 22 }, back: { color: '#d95d39', fontWeight: '800' }, title: { color: '#173b35', fontSize: 30, fontWeight: '800' }, copy: { color: '#53645f', marginBottom: 10 }, error: { color: '#b33e2e' } });
