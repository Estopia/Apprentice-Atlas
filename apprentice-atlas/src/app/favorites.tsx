...
export default function FavoritesScreen() {
  const [locale] = useLocale(); const router = useRouter(); const auth = useAuth();
  const [favorites, setFavorites] = useState<FavoriteJob[]>([]); const [loadedForUserId, setLoadedForUserId] = useState<string | null>(null); const [error, setError] = useState<FavoritesError | null>(null); const [signOutError, setSignOutError] = useState<AuthError | null>(null); const [signOutBusy, setSignOutBusy] = useState(false);
  const userId = auth.session?.user.id ?? null; const loading = isFavoritesLoading(auth.loading, userId, loadedForUserId);
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content}><View style={styles.account}><View style={styles.accountCopy}><Text style={styles.accountLabel}>{t(locale, 'saved.account')}</Text><Text style={styles.accountEmail}>{auth.user?.email ?? ''}</Text></View><Pressable accessibilityRole="button" accessibilityLabel={t(locale, 'auth.signOut')} accessibilityState={{ disabled: signOutBusy }} disabled={signOutBusy} onPress={() => void handleSignOut()} style={styles.signOut}><Text style={styles.signOutText}>{signOutBusy ? t(locale, 'auth.signingOut') : t(locale, 'auth.signOut')}</Text></Pressable></View>{signOutError && <Text accessibilityRole="alert" style={styles.error}>{getReadableAuthError(signOutError, locale)}</Text>}<Text style={styles.title}>{t(locale, 'saved.title')}</Text>{mappedError && <Text accessibilityRole="alert" style={styles.error}>{mappedError}</Text>}{!favorites.length && error ? <State text={mappedError ?? t(locale, 'saved.errorLoad')} action={t(locale, 'discovery.retry')} onPress={() => router.replace('/favorites')} /> : !favorites.length ? <State text={t(locale, 'saved.empty')} /> : <>{favorites.map((favorite) => <FavoriteCard key={favorite.id} favorite={favorite} onRemove={() => void remove(favorite)} onOpen={() => favorite.job && router.push(`/job/${favorite.job.id}`)} locale={locale} />)}{favorites.length > 1 && <Comparison favorites={favorites} locale={locale} />}</>}</ScrollView></SafeAreaView>;
}

function FavoriteCard({ favorite, onRemove, onOpen, locale }: { favorite: FavoriteJob; onRemove: () => void; onOpen: () => void; locale: 'de' | 'en' }) {
  const job = favorite.job; const archived = t(locale, 'saved.archived'); return <View style={styles.card}><Pressable accessibilityRole="button" accessibilityLabel={job?.title ?? t(locale, 'saved.unavailable')} accessibilityState={{ disabled: !job }} disabled={!job} onPress={onOpen} style={styles.cardOpen}><Text style={styles.cardTitle}>{job?.title ?? t(locale, 'saved.unavailable')}</Text><Text style={styles.company}>{job?.company ?? archived}</Text><Text style={styles.meta}>{job ? `${job.city}, ${job.country} · ${job.jobType}` : archived}</Text><Text style={styles.date}>{job?.sourceName ?? archived} · {new Date(job?.lastSeenAt || favorite.createdAt).toLocaleDateString(locale === 'de' ? 'de-DE' : 'en-GB')}</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel={t(locale, 'saved.remove')} onPress={onRemove} style={styles.remove}><Text style={styles.removeText}>{t(locale, 'saved.remove')}</Text></Pressable></View>;
}

function Comparison({ favorites, locale }: { favorites: FavoriteJob[]; locale: 'de' | 'en' }) { return <View style={styles.compare}><Text style={styles.heading}>{t(locale, 'saved.compare')}</Text>{buildComparisonRows(favorites, locale).map((row) => <View key={row.label} style={styles.compareRow}><Text style={styles.compareLabel}>{row.label}</Text>{row.values.map((value, index) => <Text key={`${row.label}-${index}`} style={styles.compareValue}>{value}</Text>)}</View>)}</View>; }
...
const styles = StyleSheet.create({ safe: { flex: 1, backgroundColor: '#f7f5f0' }, content: { width: '100%', maxWidth: 900, alignSelf: 'center', padding: 20, paddingBottom: 100 }, account: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#e9f1ed', borderRadius: 14, padding: 13, marginBottom: 16 }, accountCopy: { flex: 1 }, accountLabel: { color: '#53645f', fontSize: 12, fontWeight: '700' }, accountEmail: { color: '#173b35', fontWeight: '800', marginTop: 3 }, signOut: { minHeight: 44, minWidth: 44, justifyContent: 'center', paddingHorizontal: 10 }, signOutText: { color: '#d95d39', fontWeight: '800' }, title: { color: '#173b35', fontSize: 30, fontWeight: '800', marginBottom: 16 }, card: { backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 10, borderWidth: 1, borderColor: '#e6e1da' }, cardOpen: { minHeight: 44, minWidth: 44, justifyContent: 'center' }, cardTitle: { color: '#173b35', fontSize: 18, fontWeight: '800' }, company: { color: '#53645f', fontWeight: '700', marginTop: 5 }, meta: { color: '#53645f', marginTop: 5 }, date: { color: '#71827b', marginTop: 8, fontSize: 12 }, remove: { alignSelf: 'flex-start', marginTop: 10, minHeight: 44, minWidth: 44, justifyContent: 'center', paddingHorizontal: 8 }, removeText: { color: '#d95d39', fontWeight: '800' }, compare: { backgroundColor: '#e9f1ed', borderRadius: 16, padding: 16, marginTop: 12 }, heading: { color: '#173b35', fontSize: 20, fontWeight: '800', marginBottom: 10 }, compareRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, borderTopWidth: 1, borderTopColor: '#d6e2db', paddingVertical: 9 }, compareLabel: { width: 80, color: '#53645f', fontWeight: '800' }, compareValue: { flex: 1, minWidth: 110, color: '#173b35' }, state: { minHeight: 220, alignItems: 'center', justifyContent: 'center', gap: 14, padding: 24 }, copy: { color: '#53645f', textAlign: 'center' }, action: { backgroundColor: '#d95d39', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 11, minHeight: 44, minWidth: 44, justifyContent: 'center' }, actionText: { color: '#fff', fontWeight: '800' }, error: { color: '#b33e2e', marginBottom: 10 } });
