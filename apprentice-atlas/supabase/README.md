...
The synchronization function claims a provider listing and its canonical job through the post-chain `upsert_job_source` RPC. The RPC serializes the `(provider, external_id)` claim with a transaction advisory lock, returns the canonical `job_id`, and preserves an existing job's `created_at` while applying fresh normalized fields. The BA adapter is intentionally disabled until the Bundesagentur confirms an official read contract; it does not make network requests or send guessed credentials.
