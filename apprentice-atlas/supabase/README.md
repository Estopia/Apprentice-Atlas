...
13. `20260714180000_index_favorites_job_id.sql` — covering index for favorite-to-job lookups.
...
- **Germany — Bundesagentur für Arbeit Jobsuche:** the BA adapter calls the BA-hosted Jobsuche endpoints at `https://rest.arbeitsagentur.de/jobboerse/jobsuche-service/pc/v4/jobs` and `/pc/v4/jobdetails/{reference}`, with `X-API-Key` supplied by the server-only `BA_API_KEY` and `angebotsart=4` (Ausbildung/Duales Studium). Search results are enriched through the detail endpoint with the full posting text, education requirement, closing date, and fallback location before normalization. Detail requests use bounded concurrency and safely retain the search record if a vacancy disappears between both calls. Enable the provider explicitly with `BA_API_ENABLED=true`; its page limit is independently capped through `BA_SYNC_MAX_PAGES` so it cannot consume the UK provider's sync budget. The adapter retains the official BA job-detail page derived from `refnr`, keeps an optional `externeUrl` as the separate application URL, and accepts only German records.
