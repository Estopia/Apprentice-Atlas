...
`db reset` applies the final pre-release chain: `supabase/migrations/001_initial_schema.sql`, `supabase/migrations/001z_preflight_source_cleanup.sql`, the locked `002_harden_schema_integrity.sql` release-candidate baseline, then `003_preserve_source_identifiers.sql` as the sole post-release hardening migration, and finally `supabase/seed.sql`. The preflight is required before locked `002` for existing imports because it repairs legacy source whitespace/blanks and normalized collisions; it is guarded and harmless on clean data. `003` completes all remaining normalization-dependent source constraints, collision repair, audit protection, and compatibility hardening after the locked `002`; do not edit `002` or insert another migration before `003`. Databases already migrated through an earlier intermediate `002` should apply the guarded `003` after bringing the schema to this chain. To inspect or validate migration state, use `npx supabase migration list --local` and `npx supabase db lint --local` where supported. Docker (or another Docker-compatible runtime) is required for the local stack.
