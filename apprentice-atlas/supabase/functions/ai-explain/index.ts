...
type Body = { jobId?: unknown; language?: unknown };
...
async function contentHash(job: Record<string, unknown>, language: Language) {
  const bytes = new TextEncoder().encode(JSON.stringify({ job, language, version: PROMPT_VERSION }));
  const digest = await crypto.subtle.digest('SHA-256', bytes);
  return [...new Uint8Array(digest)].map((item) => item.toString(16).padStart(2, '0')).join('');
}

async function callOpenAi(prompt: ReturnType<typeof explanationPrompt>) {
...
export async function handleExplainRequest(request: Request): Promise<Response> {
  if (request.method === 'OPTIONS') return options();
  if (request.method !== 'POST') return errorResponse('METHOD_NOT_ALLOWED', 'Use POST.', 405);
  let body: Body;
  try { body = await request.json() as Body; } catch { return errorResponse('INVALID_JSON', 'Request body must be JSON.', 400); }
  ...
  try {
    const db = createClient(url, serviceKey) as Db;
    ...
    const output = await callOpenAi(explanationPrompt(toPromptJob(jobResult.data), body.language));
    const generatedAt = new Date().toISOString();
    const stored = await db.from('job_ai_content').upsert({ job_id: body.jobId, language_code: body.language, content_type: 'explanation', cache_key: hash, summary: output.summary, fit_reasons: output.goodIf, considerations: output.notSoGoodIf, model: MODEL(), prompt_version: PROMPT_VERSION, source_content_hash: hash, generated_at: generatedAt, metadata: { model: MODEL() } }, { onConflict: 'job_id,language_code,cache_key' });
    if (stored.error) throw new Error('Unable to cache AI explanation.');
    return json({ jobId: body.jobId, language: body.language, ...output, generatedAt, model: MODEL(), cached: false });
  } catch (error) {
    const code = error && typeof error === 'object' && 'code' in error && typeof error.code === 'string' ? error.code : 'AI_ERROR';
    return errorResponse(code, error instanceof Error ? error.message : 'Unable to create an explanation.', code === 'AI_CONFIGURATION_ERROR' ? 503 : 500);
  }
}
